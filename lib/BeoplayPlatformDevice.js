...
class BeoplayPlatformDevice {
  constructor(platform, log, config, api) {
    ...
    this.addmute = config.addmute || false
    this.addpower = config.addpower || false
    this.addvolume = config.addvolume || false
    this.addinputs = config.addinputs || false
    this.addall = config.addall || false
    ...
    // if adding a mute button to a fan, switch or bulb, warn if not power mode
    if (this.addmute && this.mode === 'mute' && (this.type === 'fan' || this.type === 'bulb' || this.type === 'switch')) {
      this.log.info(`[%s] Adding a Mute button, however Mode is set to Mute`, name)
    }

    // if adding a power button to a speaker, warn if not mute mode
    if (this.addpower && this.mode === 'power' && this.type === 'speaker') {
      this.log.info(`[%s] Adding a Power button, however Mode is set to Power`, name)
    }
          ...
          case 'SOURCE':
            if (scope.type === 'tv') {
              if (msg.notification.data.primary) {
                ...
                scope.updateInput('None')
              }
            } else if (scope.addinputs) {
              if (msg.notification.data.primary) {
                scope.updateAddedInput(msg.notification.data.primary)
              } else {
                scope.updateAddedInput('None')
              }
            }
            break
  ...
  prepareSpeakerService() {
    ...
    if (this.addvolume) this.addExtraButtons(speakerService)
  }
  ...
  prepareBulbService() {
    ...
    if (this.addmute) this.addExtraButtons(bulbService)
  }
  ...
  prepareFanService() {
    ...
    if (this.addmute) this.addExtraButtons(fanService)
  }
  ...
  prepareSwitchService() {
    ...
    let switchService = this.beoplayAccessory.getService(Service.Switch, 'main')
    if (!switchService) {
      if (this.debug) this.log.debug('Adding switch service')
      switchService = this.beoplayAccessory.addService(Service.Switch, 'main')
    }
    ...
    if (this.addvolume || this.addmute) this.addExtraButtons(switchService)
  }

  addExtraButtons(mainService) {
    // add a volume?
    if (this.addvolume && (this.type === 'speaker' || this.type === 'switch')) {
      if (mainService.getCharacteristic(Characteristic.Volume)) {
        mainService
          .getCharacteristic(Characteristic.Volume)
          .on('set', this.setVolume.bind(this))
      } else {
        // add a volume setting (not supported in the Home app or by Siri)
        mainService
    ...
    // add a mute switch?
    if (this.addmute && (this.type === 'fan' || this.type === 'bulb' || this.type === 'switch')) {
      let muteService = this.beoplayAccessory.getService('mute')
      if (!muteService) {
        if (this.debug) this.log.debug('Adding mute switch')
        muteService = this.beoplayAccessory.addService(Service.Switch, 'mute')
      }
    ...
    // add a power switch?
    if (this.addpower && this.type === 'speaker') {
      let powerService = this.beoplayAccessory.getService('power')
      if (!powerService) {
        if (this.debug) this.log.debug('Adding power switch')
        powerService = this.beoplayAccessory.addService(Service.Switch, 'power')
      }
    ...
    // add input buttons?
    if (this.addinputs && (this.type === 'speaker' || this.type === 'fan' || this.type === 'bulb' || this.type === 'switch')) {
      let counter = 1

      this.parsedInputs.forEach((input) => {
        const name = input.name

        let inputService = this.beoplayAccessory.getService(input.apiID)
        if (!inputService) {
          inputService = this.beoplayAccessory.addService(Service.Switch, input.apiID)
        }

        inputService
          .getCharacteristic(Characteristic.On)
          .on('set', this.setInput.bind(this, counter))
      ...
      if (this.devicetype === 'speaker') {
        const name = 'None'
        const apiID = 'None'
        let inputService = this.beoplayAccessory.getService(apiID)
        if (!inputService) {
          inputService = this.beoplayAccessory.addService(Service.Switch, apiID)
        }
  ...
  getAccessory(accessoryType) {
    ...
    const deviceid = UUIDGen.generate(this.name + this.ip + this.type + ACCESSORY_PROFILE_VERSION)
  ...
  async setInputButtonState(state, callback) {
    // to complete
  }
  ...
  updateAddedInput(inputID) {
    const inputName = this.lookupInputName(inputID)
    this.log.info('[%s] Updating Input to %s', this.name, inputName)

    this.parsedInputs.forEach((input) => {
      const inputService = this.beoplayAccessory.getService(input.apiID)
      if (inputID === input.apiID) {
        // this is the selected input - turn it on
        inputService.getCharacteristic(Characteristic.On)
          .updateValue(true)
      } else {
        // this isn't the selected input - turn it off
        inputService.getCharacteristic(Characteristic.On)
          .updateValue(false)
      }
    })
