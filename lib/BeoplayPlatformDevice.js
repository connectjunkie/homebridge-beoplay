...
var tvAccessory, speakerAccessory, bulbAccessory, fanAccessory
...
class BeoplayPlatformDevice {
  ...
  prepareSpeakerService () {
    ...
    speakerAccessory = this.getAccessory('speaker')
  ...
  prepareBulbService () {
    ...
    bulbAccessory = this.getAccessory('bulb')
  ...
  prepareFanService () {
    ...
    fanAccessory = this.getAccessory('fan')
  ...
  async prepareTvService () {
    ...
    tvAccessory = this.getAccessory('tv')
  ...
  async getMuteState () {
    ...
    if (!response) {
      ...
      if (this.currentMuteState !== muted) {
        if (this.debug) this.log.debug('[%s] Mute state change: State=%s, Current State=%s', this.name, muted, this.currentMuteState)
  ...
  updateMuteState () {
    ...
    if (this.type === 'speaker') {
      speakerAccessory.getService(Service.Speaker)
        .getCharacteristic(Characteristic.Mute)
        .updateValue(this.currentMuteState)
    } else if (this.type === 'bulb') {
      bulbAccessory.getService(Service.Lightbulb)
        .getCharacteristic(Characteristic.On)
        .updateValue(!this.currentMuteState)
    } else if (this.type === 'fan') {
      fanAccessory.getService(Service.Fan)
        .getCharacteristic(Characteristic.On)
        .updateValue(!this.currentMuteState)
    } else if (this.type === 'tv') {
      tvAccessory.getService(Service.TelevisionSpeaker)
        .getCharacteristic(Characteristic.Mute)
        .updateValue(!this.currentMuteState)
    }
  }
  ...
  async getPowerState () {
    ...
    if (!response) {
      ...
      if (this.currentPowerState !== state) {
        if (this.debug) this.log.debug('[%s] Power state change: State=%s, Current State=%s', this.name, state, this.currentPowerState)
  ...
  updatePowerState () {
    ...
    if (this.type === 'speaker') {
      ...
      tvAccessory.getService(Service.Television)
        .getCharacteristic(Characteristic.Active)
        .updateValue(this.currentPowerState)
    }
  }
  ...
  async getVolume () {
    ...
    if (!response) {
      ...
      if (this.currentVolume !== volume) {
        if (this.debug) this.log.debug('[%s] Volume change: Volume=%d, Maximum Volume=%d, Current Volume=%d', this.name, volume, maxVol, this.currentVolume)
        this.currentVolume = volume
        this.log('[%s] Volume is %s%', this.name, volume)

        if (this.maxVolume !== maxVol) {
          this.maxVolume = maxVol
          this.log('[%s] Maximum volume is %s%', this.name, this.maxVolume)
        }
        return true
      }
    }
  ...
  updateVolume () {
    ...
    if (this.type === 'speaker') {
      speakerAccessory.getService(Service.Speaker)
        .getCharacteristic(Characteristic.Volume)
        .updateValue(this.currentVolume)
    } else if (this.type === 'bulb') {
      bulbAccessory.getService(Service.Lightbulb)
        .getCharacteristic(Characteristic.Brightness)
        .updateValue(this.currentVolume)
  ...
  async getInput () {
    ...
    if (!response) {
      ...
      if (this.activeInputID !== input) {
        this.activeInputID = input
        if (input) {
        ...
        if (this.activeInput !== index) {
          ...
          this.log('[%s] Input number %s is active', this.name, index)
        }
        return true
      }
    }
  ...
  async setInput (desiredInput, callback) {
    ...
    if (!response) {
      ...
      this.activeInputID = input.apiID
      this.activeInput = input.name
      callback(null, input)
    }
  }
