...
class BeoplayPlatformDevice {
  ...
  getAccessory (accessoryType) {
    ...
    let newAccessory = this.unusedDeviceAccessories.find(function (a) { return (a.context.kind === accessoryType) && (a.context.id === this.name + this.ip + this.type) })
    if (newAccessory) {
      this.unusedDeviceAccessories.splice(this.unusedDeviceAccessories.indexOf(newAccessory), 1)
    } else {
      this.log.info('Adding new accessory with id ' + (this.name + this.ip + this.type) + ' and kind ' + accessoryType)
      newAccessory = new Accessory(this.name, UUIDGen.generate(this.name + this.ip + this.type + accessoryType))
      newAccessory.context.id = this.name + this.ip + this.type
