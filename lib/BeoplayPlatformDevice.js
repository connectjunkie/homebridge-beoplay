...
class BeoplayPlatformDevice {
  constructor (accessories, log, config, api) {
    ...
    var notifyStream = got.stream(this.notifyUrl)
    notifyStream.pipe(ndjson.parse())
      .on('data', function (msg) {
        if (msg.notification.type === 'VOLUME') {
          scope.log.debug('Volume: %s, MaxVol: %s, Muted: %s',
            msg.notification.data.speaker.level,
            msg.notification.data.speaker.range.maximum,
            msg.notification.data.speaker.muted
          )
        }
      })

    notifyStream.on('end', function () {
      if (this.debug) scope.log.debug('Stream ended')
    })

    notifyStream.on('error', function (err) {
      this.log.debug('Error: %s', err.stack)
    })
  ...
  async updateStatus () {
    if (await this.getVolume()) this.updateVolume()

    if (this.mode === 'power') {
      if (await this.getPowerState()) this.updatePowerState()
    } else if (this.mode === 'mute') {
      if (await this.getMuteState()) this.updateMuteState()
    }

    if (this.type === 'tv') {
      if (await this.getInput()) this.updateInput()
    }
  }

  async setupStatus () {
    // intital setup of status
  ...
  async getMuteState () {
    if (this.debug) this.log.debug('[%s] Getting Mute State', this.name)
    ...
    if (!response) {
      ...
      if (beoplayAccessory.context.currentMuteState !== muted) {
        if (this.debug) this.log.debug('[%s] Mute state change: State=%s, Current State=%s', this.name, muted, beoplayAccessory.context.currentMuteState)
        beoplayAccessory.context.currentMuteState = muted
        this.log('[%s] Mute state is currently %s', this.name, muted ? 'MUTED' : 'NOT MUTED')
        return true
      }
    }
    return false
  ...
  updateMuteState () {
    if (this.debug) this.log.debug('[%s] Updating Mute State', this.name)
  ...
  async getPowerState () {
    if (this.debug) this.log.debug('[%s] Getting Power State', this.name)
    ...
    if (!response) {
      ...
      if (beoplayAccessory.context.currentPowerState !== state) {
        if (this.debug) this.log.debug('[%s] Power state change: State=%s, Current State=%s', this.name, state, beoplayAccessory.context.currentPowerState)
        beoplayAccessory.context.currentPowerState = state
        this.log('[%s] Power state is %s', this.name, state ? 'ON' : 'STANDBY')
        return true
      }
    }
    return false
    ...
    if (beoplayAccessory.context.currentPowerState && (power === 1 || power === true)) {
      ...
      if (this.on === 'input' && (power === 1 || power === true)) {
        ...
        if (!response) {
          ...
          this.log('[%s] Power state set to %s', this.name, powerBody.standby.powerState)
  ...
  updatePowerState () {
    if (this.debug) this.log.debug('[%s] Updating Power State', this.name)
  ...
  async getVolume () {
    if (this.debug) this.log.debug('[%s] Getting Volume', this.name)
    ...
    if (!response) {
      ...
      const maxVol = parseInt(response.body.volume.speaker.range.maximum)

      if (beoplayAccessory.context.currentVolume !== volume) {
        if (this.debug) this.log.debug('[%s] Volume change: Volume=%d, Maximum Volume=%d, Current Volume=%d', this.name, volume, maxVol, beoplayAccessory.context.currentVolume)
        beoplayAccessory.context.currentVolume = volume
        this.log('[%s] Volume is %s%', this.name, volume)

        if (beoplayAccessory.context.maxVolume !== maxVol) {
          beoplayAccessory.context.maxVolume = maxVol
          this.log('[%s] Maximum volume is %s%', this.name, beoplayAccessory.context.maxVolume)
        }
        return true
      }
    }
  ...
  updateVolume () {
    if (this.debug) this.log.debug('[%s] Updating Volume', this.name)
  ...
  async setVolumeSwitch (state, callback, isUp) {
    this.log('[%s] Volume %s pressed, current volume: %s, limit: %s', this.name, isUp ? 'Up' : 'Down', beoplayAccessory.context.currentVolume, beoplayAccessory.context.maxVolume)
  ...
  async getInput () {
    if (this.debug) this.log.debug('[%s] Getting Inputs', this.name)
    ...
    if (!response) {
      ...
      if (beoplayAccessory.context.activeInputID !== input) {
        beoplayAccessory.context.activeInputID = input
        if (input) {
        ...
        if (beoplayAccessory.context.activeInput !== index) {
          beoplayAccessory.context.activeInput = index
          this.log('[%s] Input number %s is active', this.name, index + 1)
        }
        return true
      }
    }
  ...
  updateInput () {
    if (this.debug) this.log.debug('[%s] Updating Inputs', this.name)
