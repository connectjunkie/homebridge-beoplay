...
class BeoplayPlatformDevice {
  constructor (accessories, log, config, api) {
    ...
    try {
      if (!isip(config.ip)) throw new Error(`IP address for device ${config.name} is not valid. Exiting`)
    } catch (error) {
      ...
      .on('data', function (msg) {
        switch (msg.notification.type) {
          ...
          case 'PROGRESS_INFORMATION':
            ...
            if (msg.notification.data.state !== beoplayAccessory.context.currentMediaState) {
  ...
  isApi (input) {
    const res = input.match(/^[a-z]+:\d+\.\d+\.\d+@products\.bang-olufsen\.com$/)
    if (res) {
      return true
    } else {
      return false
  ...
  prepareSmartSpeakerService () {
    ...
    beoplayAccessory = this.getAccessory('smartspeaker')
    beoplayAccessory.category = 26

    let smartSpeakerService = beoplayAccessory.getService(Service.SmartSpeaker)
    if (!smartSpeakerService) {
      if (this.debug) this.log.debug('Adding smart speaker service')
      smartSpeakerService = beoplayAccessory.addService(Service.SmartSpeaker)
    }
  ...
  getAccessory (accessoryType) {
    ...
    if (newAccessory) {
      ...
      if (newAccessory.context.kind === 'tv' || newAccessory.context.kind === 'smartspeaker') {
  ...
  async setMuteState (muted, callback) {
    ...
    if (!response) {
      ...
      beoplayAccessory.context.currentMuteState = muteBody.muted
      if (callback) {
        callback(null)
      }
    }
  ...
  updateMuteState (muted) {
    ...
    if (this.type === 'speaker' && this.mode === 'mute') {
      ...
      beoplayAccessory.getService(Service.SmartSpeaker)
        .getCharacteristic(Characteristic.Mute)
        .updateValue(beoplayAccessory.context.currentMuteState)
  ...
  updateCurrentMediaState (state) {
    if (beoplayAccessory.context.currentMediaState === state) return

    this.log.info('[%s] Updating Current Media State to %s', this.name, state)
    beoplayAccessory.context.currentMediaState = state

    if (this.type === 'smartspeaker') {
      const updatedState = this.lookupState(state)

      const smartSpeakerService = beoplayAccessory.getService(Service.SmartSpeaker)
  ...
  togglePlayPause () {
    if (beoplayAccessory.context.currentMediaState === 'play') {
      this.pauseMedia()
    } else if (beoplayAccessory.context.currentMediaState === 'pause') {
      this.playMedia()
    }
  }

  toggleMuteState () {
    if (beoplayAccessory.context.currentMuteState === true) {
      this.setMuteState(false)
    } else if (beoplayAccessory.context.currentMuteState === false) {
