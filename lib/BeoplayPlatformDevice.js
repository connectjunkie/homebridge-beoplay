...
class BeoplayPlatformDevice {
  ...
  async notificationListener () {
    ...
    notifyStream.on('end', function () {
      scope.log.debug('Stream ended - attempting to reconnect in 5 seconds')
      scope.timeDelay(5000)
      scope.notificationListener()
    })

    notifyStream.on('error', function (err) {
      scope.log.debug('Error: %s', err.stack)
      scope.log.debug('Attempting to reconnect in 5 seconds')
      scope.timeDelay(5000)
      scope.notificationListener()
    })
  }
  ...
  updateMuteState (muted) {
    if (this.debug) this.log.debug('[%s] Updating Mute State to %s', this.name, muted)
    beoplayAccessory.context.currentMuteState = muted
  ...
  updatePowerState (powerState) {
    if (this.debug) this.log.debug('[%s] Updating Power State to %s', this.name, powerState)
  ...
  updateVolume (level, maximum) {
    if (this.debug) this.log.debug('[%s] Updating Volume to %d', this.name, level)
  ...
  updateInput (inputID) {
    ...
    if (this.debug) this.log.debug('[%s] Updating Input to %s', this.name, this.lookupInputName(inputID))
