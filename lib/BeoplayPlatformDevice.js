...
class BeoplayPlatformDevice {
  constructor (platform, log, config, api) {
    ...
    if (config.debug) {
      try {
        if (!(config.debug === true || config.debug === false)) throw new Error(`Debug value ${config.debug} is invalid. Exiting`)
      } catch (error) {
        this.log.error(error.message)
        return
      }
    ...
    if (config.speakergroupinputs) {
      try {
        if (!(config.speakergroupinputs === true || config.speakergroupinputs === false)) throw new Error(`Speaker groups as inputs value ${config.speakergroupinputs} is invalid. Exiting`)
      } catch (error) {
        this.log.error(error.message)
        return
      }
    ...
    this.speakergroupinputs = config.speakergroupinputs || false
  ...
  async prepareTvService () {
    ...
    // Configure TV inputs
    if (this.speakergroupinputs) {
      this.configureSpeakerGroupsAsInputs()
    } else {
      this.configureInputsAsInputs()
    }

    this.configuredInputs.forEach((input) => {
      tvService.addLinkedService(input)
    })
  }

  configureInputsAsInputs () {
    let counter = 1
    this.parsedInputs.forEach((input) => {
      ...
      this.configuredInputs.push(inputService)
  ...
  configureSpeakerGroupsAsInputs () {
    let counter = 1
    this.parsedSpeakerGroups.forEach((speakergroup) => {
      const name = speakergroup.name
      const type = Characteristic.InputSourceType.OTHER
      const visible = speakergroup.enabled
      ...
      // if the user hasn't defined a speakergroup we will create it as hidden and not configured in case they use it later
      inputService
        .setCharacteristic(Characteristic.Identifier, counter)
        .setCharacteristic(Characteristic.ConfiguredName, name)
        .setCharacteristic(Characteristic.InputSourceType, type)
        .setCharacteristic(Characteristic.IsConfigured, visible ? Characteristic.IsConfigured.CONFIGURED : Characteristic.IsConfigured.NOT_CONFIGURED)
      ...
      this.configuredInputs.push(inputService)
      if (this.debug) this.log.debug('Added input ' + counter + ', Name: ' + name + ', Visible: ' + visible)
      counter = counter + 1
    })
  }
    ...
    if (!response) {
      ...
      response.body.sources.forEach((source) => {
        ...
        if (this.inputs.length) {
          // the user has supplied inputs - use those values if supplied
          this.inputs.forEach((input) => {
            if (input.apiID === entry.apiID) {
        ...
        if (source[1].sourceType.type === 'TV') {
          this.devicetype = 'tv'
          if (this.on === 'on') this.on = 'input'
        }
        counter = counter + 1
      })
    }
    ...
    if (!response) {
      ...
      response.body.speakerGroup.list.forEach((sg) => {
        ...
        if (this.speakergroups.length) {
          // the user has supplied speakergroups - use those values if supplied
          this.speakergroups.forEach((speakergroup) => {
            if (speakergroup.id === entry.id) {
              entry.name = speakergroup.name
              entry.enabled = true
            }
          })
        } else {
          // otherwise enable all speaker groups
          entry.enabled = true
        }
        if (this.debug) this.log.debug('Added SpeakerGroup id:' + entry.id + ', Name:' + entry.name + ', Enabled:' + entry.enabled)
        this.parsedSpeakerGroups.push(entry)
      })
    }
  }
  ...
  async getSpeakerGroup () {
    ...
    if (!response) {
      ...
      const speakergroup = response.body.speakerGroup.active

      if (this.beoplayAccessory.context.currentSpeakerGroup !== speakergroup) this.updateSpeakerGroup(speakergroup)
    }
  }

  updateSpeakerGroup (speakergroup) {
    this.log.info('[%s] Updating Speaker Group to %s', this.name, speakergroup)
    this.beoplayAccessory.context.currentSpeakerGroup = speakergroup

    if (this.type === 'tv' && this.speakergroupinputs) {
      const inputName = this.lookupSpeakerGroupName(speakergroup)
      this.log.info('[%s] Updating Speaker Group to %s', this.name, inputName)
      ...
      // user has selected an input that isn't shown in HomeKit, so add it
        this.log.warn('[%s] Enabling disabled Speaker Group: %s', this.name, inputName)
      ...
      this.beoplayAccessory.getService(Service.Television)
        .getCharacteristic(Characteristic.ActiveIdentifier)
        .updateValue(this.lookupSpeakerGroupIndex(speakergroup) + 1)
    }
  }
  ...
  async setPowerState (power, callback) {
    ...
    if (this.beoplayAccessory.context.currentPowerState && (power === 1 || power === true)) {
      ...
      if (this.on === 'input' && (power === 1 || power === true)) {
        ...
        this.setInput(this.default, callback, true)
  ...
  nextSpeakerGroup (index) {
    const current = Number.isInteger(index) ? index : this.lookupSpeakerGroupIndex(this.beoplayAccessory.context.currentSpeakerGroup)
    if ((current + 1) === this.parsedSpeakerGroups.length) {
      if (this.parsedSpeakerGroups[0].enabled === true) {
        this.setSpeakerGroup(this.parsedSpeakerGroups[0].id)
      } else {
        this.nextSpeakerGroup(0)
      }
    } else {
      if (this.parsedSpeakerGroups[current + 1].enabled === true) {
        this.setSpeakerGroup(this.parsedSpeakerGroups[current + 1].id)
      } else {
        this.nextSpeakerGroup(current + 1)
      }
    }
  }

  prevSpeakerGroup (index) {
    const current = Number.isInteger(index) ? index : this.lookupSpeakerGroupIndex(this.beoplayAccessory.context.currentSpeakerGroup)
    if ((current - 1) <= 0) {
      const last = this.speakergroups.length - 1
      if (this.parsedSpeakerGroups[last].enabled === true) {
        this.setSpeakerGroup(this.parsedSpeakerGroups[last].id)
      } else {
        this.nextSpeakerGroup(last)
      }
    } else {
      if (this.parsedSpeakerGroups[current - 1].enabled === true) {
        this.setSpeakerGroup(this.parsedSpeakerGroups[current - 1].id)
  ...
  async setInput (desiredInput, callback, poweron) {
    if (!this.speakergroupinputs || poweron) {
      // if setting an input normally
      ...
      // we have speakergroups as inputs
      const id = this.parsedSpeakerGroups[desiredInput - 1]
      this.setSpeakerGroup(id)
    }
  }

  updateInput (inputID) {
    // if we have speaker groups as inputs instead of actual inputs, return
    if (this.speakergroupinputs) return
