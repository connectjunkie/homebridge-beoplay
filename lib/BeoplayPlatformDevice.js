...
class BeoplayPlatformDevice {
  ...
  async setVolume (volume, callback) {
    ...
    if (volume > this.beoplayAccessory.context.maxVolume) {
      if (this.debug) this.log.debug('[%s] Volume of %d is higher than the device max, setting to the max of %d', this.name, volume, this.beoplayAccessory.context.maxVolume)
    ...
    if (!response) {
      ...
      if (originalvolume > volume) {
        if (this.debug) this.log.debug('[%s] Adjusting HomeKit device volume to %d', this.name, volume)
        this.updateVolume(volume, this.beoplayAccessory.context.maxVolume)
      }
  ...
  updateVolume (level, maximum) {
    if (this.maxvolumescaling) {
      ...
      this.log.info('[%s] Updating Volume to %d and max volume to %d', this.name, level, maximum)
    }
