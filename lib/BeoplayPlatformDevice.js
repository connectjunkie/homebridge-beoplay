...
class BeoplayPlatformDevice {
  ...
  setupAccessoryServices () {
    ...
    const deviceid = this.name + this.ip + this.type
  ...
  prepareFanService () {
    this.log.info('Creating fan')

    let fanAccessory = this.unusedDeviceAccessories.find(function (a) { return a.context.kind === 'fanAccessory' })
    if (fanAccessory) {
      this.unusedDeviceAccessories.splice(this.unusedDeviceAccessories.indexOf(fanAccessory), 1)
    } else {
      this.log.info('Adding new accessory with id ' + (this.name + this.ip + this.type) + ' and kind fanAccessory.')
      fanAccessory = new Accessory(this.name, UUIDGen.generate(this.name + this.ip + this.type + 'fanAccessory'))
      fanAccessory.context.id = this.name + this.ip + this.type
      fanAccessory.context.kind = 'fanAccessory'
      this.newDeviceAccessories.push(fanAccessory)
    }
    this.deviceAccessories.push(fanAccessory)
