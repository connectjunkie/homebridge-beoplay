...
class BeoplayPlatformDevice {
  ...
  getAccessory (accessoryType) {
    ...
    let newAccessory = this.platform.accessories.find(function (a) { return (a.context.id === deviceid) })
    if (newAccessory) {
  ...
  updateVolume (level, maximum) {
    ...
    if (this.type === 'speaker') {
      // const speakerService = this.beoplayAccessory.getService(Service.Speaker)
